...
export default function ReactLeafletSearch(props) {
    ...
    let handler = ({ event, payload }) => {
        var _a;
        if (event === "add" && payload) {
            (_a = props.onChange) === null || _a === void 0 ? void 0 : _a.call(props, payload);
            ...
            setState(s => (Object.assign(Object.assign({}, s), { search: false })));
        }
    };
    let extraCpt = searchInfo && state.search ? (props.children ? (props.children(searchInfo)) : props.showMarker ? (React.createElement(Marker, Object.assign({ key: `marker-search-${state.search.toString()}`, position: state.search }, (props.markerIcon ? { icon: props.markerIcon } : {})), props.showPopup && props.popUp ? (props.popUp(searchInfo)) : (React.createElement(Popup, null,
        React.createElement("span", null, state.info))))) : null) : null;
    return (React.createElement("div", null,
        React.createElement(SearchControl, Object.assign({ className: props.className, provider: props.provider, customProvider: props.customProvider, providerOptions: props.providerOptions, openSearchOnLoad: props.openSearchOnLoad, closeResultsOnClick: props.closeResultsOnClick, inputPlaceholder: props.inputPlaceholder, search: props.search, map: context.map, handler: handler }, (props.tabIndex !== undefined ? { tabIndex: props.tabIndex } : {}))),
        extraCpt));
}
